...
def run_merge_visible_points_in_selected_polygon(iface):
    """
    QGIS plugin の def run(self) から呼び出すための関数
    iface を受け取り、選択ポリゴン内にある表示中ポイントを結合して
    ...
    try:
        ...
        if not visible_point_layers:
            raise Exception("表示されているポイントレイヤが見つかりません。")
        ...
        out_layer = QgsVectorLayer(
            f"Point?crs={polygon_crs.authid()}",
            output_layer_name,
            "memory"
        )
        ...
        if not new_features:
            raise Exception("選択ポリゴン内にある表示中ポイント feature は見つかりませんでした。")
        ...
        if saved_layer.isValid():
            ...
            iface.messageBar().pushSuccess("完了", "ポイント結合レイヤを保存して追加しました。")
